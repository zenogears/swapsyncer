...
if not os.path.exists(eboots_path):
	...
	for fn in download_list:
		...
		datchik = int(palka)
		update(datchik,"Обновляем базу, подождите... Это может занять длительное время. Скачано: " + str("%.2f" % palka) + " %")
		r = requests.get(url + "/SWAP/" + fn)
		with open(eboots_path + '/' + fn, 'wb') as fd:
			fd.write(r.content)
			fd.close()
	if not len(download_list)==0:
		update(100)
	...
	for fn in upload_list:
		...
		palka = float(palka) + float(testparam)
		datchik = int(palka)
		update(datchik,"Загружаем данные на сервер, подождите... Это может занять длительное время. Загружено: " + str("%.2f" % palka) + " %")
